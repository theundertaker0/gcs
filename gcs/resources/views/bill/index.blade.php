...
<script>
    ...
    $(document).ready(function() {
        $('#tblBills').DataTable({
            ...
            "ajax": "{{ url('/facturas') }}",
            "columns": [
                {data: 'folio', name: 'folio',width:"30%"},
                ...
                },
                {
                    ...
                    render:function(data,type,full,meta){
                        return '<form action="{{url("bill")}}/'+data.id+'" method="post" style="max-width:45x;">'
                            +'<button class="btn btn-secondary" data-toggle="tooltip" title="Ver artículos"><span class="fas fa-eye"></span></button>&nbsp;'+
                            '<button class="btn btn-secondary" data-toggle="tooltip" title="Agregar Artículo"><span class="fas fa-plus"></span></button>&nbsp;'+
